...
// 批改提交必须写回云端；刷新后由云端 status 决定待批改或已批改。
setTimeout(()=>{$('#submitGrading').onclick=async()=>{const p=state.papers.find(x=>x.id===selectedId);if(!p)return toast('请先选择一份试卷');const grading=total();if(grading.bigTotal>70)return toast('综合应用题总分不能超过 70 分');try{await cloudRequest(`/rest/v1/papers?id=eq.${p.id}`,{method:'PATCH',headers:{'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({choice_scores:grading.choice,big_scores:grading.big,score:grading.score,comment:$('#commentInput').value.trim(),strokes:strokes,status:'graded',graded_at:new Date().toISOString()})});selectedId=null;$('#graderWorkspace').classList.add('hidden');$('#workspacePlaceholder').classList.remove('hidden');await loadCloud();toast('批改已提交并保存到云端')}catch(error){console.error(error);toast('批改提交失败，请重试')}}},90);
// 学生照片上传前压缩：最长边 1600px，目标单张不超过 0.5MB。
async function compressExamPhoto(file){if(!file.type.startsWith('image/'))return file;const image=await createImageBitmap(file);let scale=Math.min(1,1600/Math.max(image.width,image.height));let width=Math.max(1,Math.round(image.width*scale)),height=Math.max(1,Math.round(image.height*scale));const makeBlob=(quality)=>new Promise(resolve=>{const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;canvas.getContext('2d').drawImage(image,0,0,width,height);canvas.toBlob(resolve,'image/jpeg',quality)});let blob;for(const quality of [.82,.72,.62,.52,.42]){blob=await makeBlob(quality);if(blob?.size<=512*1024)break;if(quality===.42&&Math.max(width,height)>800){width=Math.round(width*.8);height=Math.round(height*.8)}}image.close?.();if(!blob)throw new Error('图片压缩失败');return new File([blob],file.name.replace(/\.[^.]+$/,'.jpg'),{type:'image/jpeg'})}
setTimeout(()=>{$('#studentUploadForm').onsubmit=async e=>{e.preventDefault();const rawFiles=[...$('#fileInput').files],name=$('#studentName').value.trim(),phone=$('#studentPhone').value.trim();if(!rawFiles.length)return toast('请选择试卷文件');const button=e.submitter;button.disabled=true;button.textContent='正在压缩图片…';try{const files=await Promise.all(rawFiles.map(compressExamPhoto));if(files.some(f=>f.size>20*1024*1024))throw new Error('文件超过 20MB');button.textContent='正在上传…';const id=crypto.randomUUID(),paths=files.map(f=>`${id}/${crypto.randomUUID()}-${f.name.replace(/[^\\w.\\-\u4e00-\u9fa5]/g,'_')}`);await Promise.all(files.map(async(f,i)=>{const response=await fetch(`${cloudUrl}/storage/v1/object/paper-files/${paths[i].split('/').map(encodeURIComponent).join('/')}`,{method:'POST',headers:{...cloudHeaders,'Content-Type':f.type,'x-upsert':'false'},body:f});if(!response.ok)throw new Error(await response.text())}));await cloudRequest('/rest/v1/papers',{method:'POST',headers:{'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({id,student_name:name,student_phone:phone,file_path:paths[0],file_paths:paths,file_name:files.map(f=>f.name).join('、'),mime_type:files[0].type})});$('#studentFormView').classList.add('hidden');$('#studentSuccess').classList.remove('hidden');$('#submittedDetails').textContent=`${name} · 计算机 408 考试 · 共 ${files.length} 页（图片已压缩）`;await loadCloud()}catch(error){console.error(error);toast(`上传失败：${String(error.message||error).slice(0,80)}`)}finally{button.disabled=false;button.textContent='确认提交试卷 →'}}},120);
